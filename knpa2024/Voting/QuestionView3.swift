...
class QuestionView3: UIView {
    ...
    init(frame: CGRect, titleName : String, mainVC kMainVC : UIViewController) {
        ...
        let innerView1 = UIView(frame: CGRect(x: 0, y: naviBar.frame.maxY, width: SCREEN.WIDTH, height: self.frame.size.height - naviBar.frame.maxY - 10))
        ...
        selectButton.setTarget(event: .touchUpInside) { [weak self] (butotn) in
            ...
            if self.questionSelectView == nil {
                ...
                self.questionSelectView?.questionView3 = self
